...
function cargar_lista(){
 $("#lista_de_usuarios").load("ver_administradores.php",function(){
...
function ingresar_datos(){
 var form= $("#usuarios").serialize();
  $.post('insertar_administradores.php',form,
		function(data){
...
function edit_user(user){
  $("#iden").val(user);
   $("#enviar").val('Editar');
  $.get('informacion_administrador.php?iden='+user,{
    type: 'xml'
  },
  function(xml){
	 ...
	 $("#tipo_admin").val($(xml).find('tipo_admin').text());
...
function user_edit_submit(user){
	var form = $("#usuarios").serialize();
	    $.post('editar_administrador.php',form,
		function(data){
     	        $("#usuarios").hide('medium');
	            cargar_lista();
			}
...
function activate(set){
	...
	arr = jQuery.map(arr, function(n, i){
		return ('administrador_id=' + n.value);
	});
				
	users = arr.join(" or ");
	
	$.post('activate_administrators.php',{
			condition: users,
			value: set
		},
		function(data){
...
function delete_users()
{
	 if($("#lista_de_usuarios input[type=checkbox]:checked").length >= 1)
	 {
		...
		{   
			...
			$.post('eliminar_administradores.php',{
					condition: users
				},
				function(data){
						cargar_lista();
