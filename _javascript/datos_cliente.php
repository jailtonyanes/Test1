  ...
  mysql_select_db($dbname);
   ...
   $query="SELECT
    preguntas.pregunta_contenido
    , preguntas.pregunta_id
    , encuesta.encuesta_nombre
	    ,encuesta.encuesta_id
FROM
    encuesta.preguntas
    INNER JOIN encuesta.encuesta 
        ON (preguntas.encuesta_id = encuesta.encuesta_id) WHERE pregunta_id='$_GET[uid]'";
...
?>
  ...
  <p>
    <label>* Encuesta:
      ...
      <select name="encuesta" id="encuesta" class="ancho3">
        <option value="0" selected="selected">Escoja una Opci&oacute;n</option>
        <?php 
	    $connection = mysql_connect($server,$user,$password);
		mysql_select_db($dbname);
	    $query="SELECT * FROM encuesta ORDER BY encuesta_nombre asc";
			  ...
			  if($num_rows>0)
			   {
				   while($row2=mysql_fetch_array($result2))
				    {
	   ?>
        <option value="<?php echo $row2['encuesta_id']  ?>" <?php if($x==$row2['encuesta_id']){ echo 'selected="selected"';}  ?> ><?php echo $row2['encuesta_nombre'] ?></option>
 ...
 <p>
    <label>* Pregunta:
      <textarea class="ancho3" name="pregunta" id="pregunta" cols="3" rows="3"><?php echo $row['pregunta_contenido'] ?></textarea>
    </label>
  </p>
  ...
  <!-- <p>
    <label>* Precio:
      <input type="text" name="precio" id="precio" class="ancho" onKeyPress="return validarnum(event)" value="<?php // echo $row['zona_precio']; ?>"/>
    </label>
  </p> !-->
