...
function ver_clientes(){
 ...
 $("#view_cli").load("sacar_items.php",function(){
...

{
...

	 {
		...
		{   
			...
			return ('subcategoria_id=' + n.value);
...
function delete_items()
{
	 if($("#datos_cli input[type=checkbox]:checked").length >= 1)
	 {
		...
		{   
			...
			var arr = $("#datos_cli input[type=checkbox]:checked");
			arr = jQuery.map(arr, function(n, i){
			return ('item_id=' + n.value);
			});
			users = arr.join(" or ");
			$.post('eliminar_items.php',{
					condition: users
				},
				function(data){
						ver_clientes();
...
$(document).ready(function(){
   ...
   $("#guardar").bind('click',function(){
       ...
       if($("#subgategoria_nombre").val()=='' || $("#categoria_id").val()=='' )
