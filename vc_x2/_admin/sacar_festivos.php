...
?>
 ...
 <table width="auto" border="0" id="datos_cli" cellpadding="0" cellspacing="0">
  ...
  if($_GET['fecha']!='')
  {
   $fecha= " WHERE YEAR(fecha_festiva)='$_GET[fecha]' "; 
  }
  else
  {
    $fecha='';
   ...
   $crud->setConsulta("SELECT * FROM festivo $fecha ORDER BY fecha_festiva  desc");
  ...
  <tr>
    ...
    <td align="center" class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>>
      ...
      <input type="checkbox" name="selected[]" id="identy<?php  echo $datos1[$i]['fecha_id']  ?>" value="<?php echo $datos1[$i]['fecha_id']  ?> " /> 

    </td>
    <td align="center" class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><?php echo $datos1[$i]['fecha_festiva'] ?></td>
    
    
 <td align="center" class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><a onclick="javascript: subir();editar_cliente(<?php echo $datos1[$i]['fecha_id'] ?>)" style="cursor:pointer;color:#A0BBD8;"><img src="../_images/edit.ico" alt="Edit Icon"  ></a> </td>
  </tr>
