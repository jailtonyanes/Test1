 <?php
  ...
  $crud2 = new Crud();
     ...
     $crud2-> setConsulta("SELECT municipio.`municipio_id`,municipio.`municipio_nombre`, sucursal.`sucursal_id`,sucursal.`sucursal_nombre`
 FROM municipio
...
JOIN cliente ON (cliente.`cliente_id`=sucursal.`cliente_id`) WHERE departamento.`departamento_id`='$_GET[uid1]' AND cliente.`cliente_id`
='$_GET[uid2]'  GROUP BY municipio.`municipio_id`  order by municipio_nombre asc");
