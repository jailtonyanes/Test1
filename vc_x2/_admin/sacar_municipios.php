...
FROM
   ...
   INNER JOIN `municipio` 
       ON (`departamento`.`departamento_id` = `municipio`.`departamento_id`) ORDER BY departamento_nombre ASC, municipio_nombre asc");
  ...
  <tr>
    ...
    <td align="center" class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>>
      ...
      <input type="checkbox" name="selected[]" id="identy<?php  echo $datos1[$i]['municipio_id']  ?>" value="<?php echo $datos1[$i]['municipio_id']  ?> " /> 
 ...
 <td align="center" class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><a onclick="javascript: subir();editar_cliente(<?php echo $datos1[$i]['municipio_id'] ?>)" style="cursor:pointer;color:#A0BBD8;"><img src="../_images/edit.ico" alt="Edit Icon"  ></a> </td>
  </tr>
