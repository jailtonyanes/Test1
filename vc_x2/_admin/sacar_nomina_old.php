...
, `hora_rec_noct_dom`, tipo_programacion  from calcu_nom where fecha>='$_GET[f1]' and fecha<= '$_GET[f2]' $guarda");
  ...
  <tr>
   ...
   <!--  <td>
    ...
    <td <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><?php echo  $cont_ext.htmlentities($datos1[$i]['tipo_turno']).'dia_sem'.date("w", strtotime($datos1[$i]['fecha']))?></td>
    ...
    <td align="right" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><?php
      ...
      if($datos1[$i]['tipo_turno']=='D')
      {
        ...
        if($cont_ext <=7 && $datos1[$i]['difdia']*$cont_ext>48)
        {
            $cont_ext=0;
            //calculo con extras en festivo
                if(date("w", strtotime($datos1[$i]['fecha']))=='0' || $datos2[0]['coinc']== 1)
                   {
                      echo number_format($datos1[$i]['ext_fest_diur']*$datos1[$i]['difdia']+$sub_t,0,'.',',');
                   ...
                   {// dia ordinario
                     echo number_format($datos1[$i]['ext_diur']*$datos1[$i]['difdia']+$sub_t,0,'.',','); 
        ...
        else
        {
           ...
           //dia festivo o domingo diurno
                   if(date("w", strtotime($datos1[$i]['fecha']))=='0' || $datos2[0]['coinc']== 1)
                   {
                      echo number_format($datos1[$i]['hora_ord_dom_diur']*$datos1[$i]['difdia']+$sub_t,0,'.',',');
                   ...
                   {// dia ordinario
                     echo number_format($datos1[$i]['vlr_hora']*$datos1[$i]['difdia']+$sub_t,0,'.',','); 
      ...
      else
      {
        if($datos1[$i]['tipo_turno']=='N')
        {
             ...
             if($cont_ext <=7 && $datos1[$i]['difdia']*$cont_ext>48)//CASO CON EXTRAS
             {
                           $cont_ext=0;
                            //domingo o festivo nocturno 
                                 if(date("w", strtotime($datos1[$i]['fecha']))=='0' || $datos2[0]['coinc']== 1)
                                 {
                                    //noche  festiva sin recargo
                                       ...
                                       echo number_format($exdf+$exnf,0,'.',',');    
                                       $tot_ex_f_n=+($exnf);
                                       $c_exnoctf=+$a; 
                                       $tot_ext_f_d=$exdf;
                                       $cot_ext_f_d=(12-$a); 
                                 ...
                                 if(date("w", strtotime($datos1[$i]['fecha']))=='0' || $datos2[0]['coinc']== 1)
                                 {
                                   ...
                                   if($a <= 1)
                                     ...
                                     {//noche festiva con recargo
                                        echo number_format(($datos1[$i]['hora_ord_dom_diur']*$datos1[$i]['difdia'])+($a*$datos1[$i]['hora_rec_noct_dom'])+$sub_t,0,'.',',');
                                   ...
                                   if($a <= 1)
                                    {
                                     ...
                                     {  //recargo de las noches
                                        ...
                                        echo number_format(($datos1[$i]['vlr_hora'] * $datos1[$i]['difdia'])+($a*$datos1[$i]['rec_noct'])+$sub_t,0,'.',',');
                                       
                                        $tot_rec_noct=+($datos1[$i]['vlr_hora'] * $datos1[$i]['difdia'])+($a*$datos1[$i]['rec_noct']) + $sub_t;

                                     }
                                 }
                  //FIN CASO HORAS EXTRAS
             }
        ...
        else
        {
         ...
         //para ver si las horas de los descansos son a 8 horas o 6 horas
...
<!-- extras diurnas !-->
<input type="text" name="exd" id="exd" value="<?php echo $cot_ext_d;  ?>"></input>
<input type="text" name="texd" id="texd" value="<?php echo $tot_ext_d;  ?>"></input>

<!-- extras festivas diurnas !-->
<input type="text" name="exdf" id="exdf" value="<?php echo  $cot_ext_f_d;  ?>"></input>
<input type="text" name="texdf" id="texdf" value="<?php echo  $tot_ext_f_d; ?>"></input>

<!-- extras festivas noctunras !-->
<input type="text" name="exnoctf" id="exnoctf" value="<?php echo $c_exnoctf  ?>"></input>
<input type="text" name="texnf" id="texnf" value="<?php echo $tot_ex_f_n ?>"></input>

<!-- extras  noctunras !-->

<input type="text" name="exnoct" id="exnoct" value="<?php echo $c_exnoct  ?>"></input>
<input type="text" name="texn" id="texn" value="<?php echo $tot_ex_n ?>"></input>

<!-- extras Recargos noctunras !-->
<input type="text" name="fest_noche" id="fest_noche" value="<?php echo $cot_nochef_cr ?>"></input>
<input type="text" name="cont_fest_noche" id="cont_fest_noche" value="<?php echo $tot_nochef_cr ?>"></input>


<!-- extras dominical diurno noctunras !-->

<input type="text" name="fest_dia" id="fest_dia" value="<?php echo $tot_domd  ?>"></input>
<input type="text" name="cont_fest_dia" id="cont_fest_dia" value="<?php echo $cont_domd ?>"></input>
