...
<input type="hidden" name="docu" id="docu" />
  <table width="800" border="0" id="datos_cli" cellpadding="0" cellspacing="0" style="margin:5px auto">
    <?php
			  $query="SELECT * FROM preguntas WHERE encuesta_id =7 AND pertenece_a = 'MR2'";
  ...
  if($result)
   {
    ...
    <tr>
      ...
      <td class="margen_der_abj" <?php if( $i % 2 !=0){echo 'style="background-color:#F0F0F6" ';} ?>><select name="<?php echo $row['pregunta_id'] ?>" id="<?php echo $row['pregunta_id'] ?>" onchange="javascript:percenta(<?php echo $row['pregunta_id'] ?>)"  class="tipo">
    ...
    <tr><td colspan="2"><label>Porque:</label><input name="pq" type="text" id="pq" value="N/A" size="144" /></td></tr>
    <?php
		   $i++;
		 }
		 ?>
